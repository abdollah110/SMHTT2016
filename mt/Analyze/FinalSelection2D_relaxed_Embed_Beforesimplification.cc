...
//AM
       bool isSingleLepEmbedded = (passIsoMu22) or (passIsoTkMu22) or (passIsoMu22eta2p1) or (passIsoTkMu22eta2p1);
        if (sample=="data_obs" && run<278820 && !id_m_medium2016_1) continue;
        if (sample=="data_obs" && run>=278820 && !id_m_medium_1) continue;

//        if (sample=="embedded" && genDR_2>0.2) continue; //AM

       
       //###############################################################
       // Remove events with moun pt less than 23 GeV
       if (pt_1<23) continue;
...
//	if (sample!="embedded" && (!((isSingleLep && pt_1>23) or (isCrossTrigger && pt_1<=23)))) continue;
           ...
           if ( !isSingleLepEmbedded) continue;
...
//        if (sample=="embedded" && amcatNLO_weight>1) amcatNLO_weight=0.10; //AM
...
//AM//	bool is_bveto=(sample!="data_obs" or nbtag==0);
       bool is_bveto=(sample!="data_obs" or nbtag==0);
//         is_bveto=(sample!="data_obs" or nbtag==0);
...
//        if (EmbedWeight < 0.5)  cout<<muon_id_scalefactor<<"  "<<muon_iso_scalefactor<<"  "<<muon_trg_efficiency<<"  \n";
	    ...
	    if (qcdCR && q_1*q_2<0 && mt<50){
	        if (is_0jet)
	    	    h0_CR_QCD[k]->Fill(var2,Total_Embed_Weight);
	        if (is_boosted)
                    h1_CR_QCD[k]->Fill(var2,Total_Embed_Weight);
	        if (is_VBF)
                    h2_CR_QCD[k]->Fill(var2,Total_Embed_Weight);
	    }
            ...
            if (qcdCR && q_1*q_2>0 && mt<50){
                if (is_0jet)
                    h0_CRSS_QCD[k]->Fill(var2,Total_Embed_Weight);
                if (is_boosted)
                    h1_CRSS_QCD[k]->Fill(var2,Total_Embed_Weight);
                if (is_VBF)
                    h2_CRSS_QCD[k]->Fill(var2,Total_Embed_Weight);
            }
	    ...
	    if (is_0jet && mt<50){
                ...
                if (signalRegion && q_1*q_2<0){
		    pteta_0jet->Fill(mytau.Pt(),mytau.Eta(),Total_Embed_Weight);
		    mUESUp_0jet->Fill((m_sv_UESUp-m_sv)/m_sv_UESUp,Total_Embed_Weight);
                    mUESDown_0jet->Fill((m_sv_UESDown-m_sv)/m_sv_UESDown,Total_Embed_Weight);
                    mCESUp_0jet->Fill((m_sv_JESUp-m_sv)/m_sv_JESUp,Total_Embed_Weight);
                    mCESDown_0jet->Fill((m_sv_JESDown-m_sv)/m_sv_JESDown,Total_Embed_Weight);
                    mTESUp_0jet->Fill((m_sv_UP-m_sv)/m_sv_UP,Total_Embed_Weight);
                    mTESDown_0jet->Fill((m_sv_DOWN-m_sv)/m_sv_DOWN,Total_Embed_Weight);
                    h0_OS[k]->Fill(var1_0,var2,Total_Embed_Weight);
                    if (l2_decayMode==0) h2D_0jet->Fill(var2,0.0,Total_Embed_Weight);
                    if (l2_decayMode==1) h2D_0jet->Fill(var2,1.0,Total_Embed_Weight);
                    if (l2_decayMode==10) h2D_0jet->Fill(var2,2.0,Total_Embed_Weight);
		}
                if (signalRegion && q_1*q_2>0)
                    h0_SS[k]->Fill(var1_0,var2,Total_Embed_Weight);
                if (wRegion && q_1*q_2<0)
                    h0_WOS[k]->Fill(var1_0,var2,Total_Embed_Weight);
                if (wRegion && q_1*q_2>0)
                    h0_WSS[k]->Fill(var1_0,var2,Total_Embed_Weight);
                if (qcdRegion && q_1*q_2>0)
                    h0_QCD[k]->Fill(var1_0,var2,Total_Embed_Weight);
	    }
...
HiggspT->Fill(pt_sv,(mymu+mytau+mymet).Pt());
                ...
                if (signalRegion && q_1*q_2<0){
                    ...
                    h2D_boosted->Fill(var2,var1_2,Total_Embed_Weight);
		    if (var1_1>150) {
                    mUESUp_boosted->Fill((m_sv_UESUp-m_sv)/m_sv_UESUp,Total_Embed_Weight);
                    mUESDown_boosted->Fill((m_sv_UESDown-m_sv)/m_sv_UESDown,Total_Embed_Weight);
                    mCESUp_boosted->Fill((m_sv_JESUp-m_sv)/m_sv_JESUp,Total_Embed_Weight);
                    mCESDown_boosted->Fill((m_sv_JESDown-m_sv)/m_sv_JESDown,Total_Embed_Weight);
		    ...
		    if (var1_1<100) pteta_boosted_1->Fill(mytau.Pt(),mytau.Eta(),Total_Embed_Weight);
                    else if (var1_1<150) pteta_boosted_2->Fill(mytau.Pt(),mytau.Eta(),Total_Embed_Weight);
                    else if (var1_1<200) pteta_boosted_3->Fill(mytau.Pt(),mytau.Eta(),Total_Embed_Weight);
                    else if (var1_1<250) pteta_boosted_4->Fill(mytau.Pt(),mytau.Eta(),Total_Embed_Weight);
                    else if (var1_1<300) pteta_boosted_5->Fill(mytau.Pt(),mytau.Eta(),Total_Embed_Weight);
                    else pteta_boosted_6->Fill(mytau.Pt(),mytau.Eta(),Total_Embed_Weight);
		}
                if (signalRegion && q_1*q_2>0)
                    h1_SS[k]->Fill(var1_1,var2,Total_Embed_Weight);
                if (wRegion && q_1*q_2<0)
                    h1_WOS[k]->Fill(var1_1,var2,Total_Embed_Weight);
                if (wRegion && q_1*q_2>0)
                    h1_WSS[k]->Fill(var1_1,var2,Total_Embed_Weight);
                if (qcdRegion && q_1*q_2>0)
                    h1_QCD[k]->Fill(var1_1,var2,Total_Embed_Weight);
            }
...
//if (mjj>1500) cout<<run<<" "<<lumi<<" "<<evt<<" "<<m_sv<<" "<<pt_1<<" "<<pt_2<<" "<<mjj<<endl;
                    ...
                    h2D_vbf->Fill(var2,var1_2,Total_Embed_Weight);
                    if (var1_2<700) pteta_vbf_1->Fill(mytau.Pt(),mytau.Eta(),Total_Embed_Weight);
                    else if (var1_2<1100) pteta_vbf_2->Fill(mytau.Pt(),mytau.Eta(),Total_Embed_Weight);
                    else if (var1_2<1500) pteta_vbf_3->Fill(mytau.Pt(),mytau.Eta(),Total_Embed_Weight);
                    else pteta_vbf_4->Fill(mytau.Pt(),mytau.Eta(),Total_Embed_Weight);
                    mUESUp_vbf->Fill((m_sv_UESUp-m_sv)/m_sv_UESUp,Total_Embed_Weight);
                    mUESDown_vbf->Fill((m_sv_UESDown-m_sv)/m_sv_UESDown,Total_Embed_Weight);
                    mCESUp_vbf->Fill((m_sv_JESUp-m_sv)/m_sv_JESUp,Total_Embed_Weight);
                    mCESDown_vbf->Fill((m_sv_JESDown-m_sv)/m_sv_JESDown,Total_Embed_Weight);
                ...
                if (wRegion && q_1*q_2<0)
                    h2_WOS[k]->Fill(var1_2,var2,Total_Embed_Weight);
                if (wRegion && q_1*q_2>0)
                    h2_WSS[k]->Fill(var1_2,var2,Total_Embed_Weight);
                if (qcdRegion && q_1*q_2>0)
                    h2_QCD[k]->Fill(var1_2,var2,Total_Embed_Weight);
            }
	}
